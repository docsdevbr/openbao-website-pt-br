...
description: |-
  ...
  recovery token for OpenBao.  They are only active in recovery mode.
---
...
is reached, OpenBao will complete the recovery token generation and issue the new
...
Note that once a token has been issued, OpenBao is unsealed. The token lives
