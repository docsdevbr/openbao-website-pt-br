...
description: The `/sys/raw` endpoint is used to access the raw underlying store in OpenBao.
---

# `/sys/raw`

The `/sys/raw` endpoint is used to access the raw underlying store in OpenBao.
...
- `path` `(string: <required>)` – Specifies the raw path in the storage
  backend. This is specified as part of the URL.
