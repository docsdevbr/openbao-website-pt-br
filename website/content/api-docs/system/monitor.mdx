...
description: The `/sys/monitor` endpoint is used to receive streaming logs from the OpenBao server.
---

# `/sys/monitor`

The `/sys/monitor` endpoint is used to receive streaming logs from the OpenBao server.

If OpenBao is emitting log messages faster than a receiver can process them, then
some log lines will be dropped.

## Monitor system logs

This endpoint streams logs back to the client from OpenBao. Note that unlike most API endpoints in OpenBao, this one
