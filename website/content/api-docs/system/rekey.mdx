...
description: The `/sys/rekey` endpoints are used to rekey the unseal keys for OpenBao.
---
...
Note: As of v2.4.0 usage of these endpoints is [deprecated](/docs/deprecation/unauthed-rekey)
and will be disabled by default. For the new authenticated version of the rekey
endpoints, please see [rotation endpoints](rotate/index.mdx).

:::

The `/sys/rekey` endpoints are used to rekey the unseal keys for OpenBao.
...
of the OpenBao. If the threshold number of root key shares is reached, OpenBao
