...
sidebar_label: PKI
description: This is the API documentation for the OpenBao PKI secrets engine.
---
...
This is the API documentation for the OpenBao PKI secrets engine. For general
...
- [OpenBao CLI with DER/PEM responses](#openbao-cli-with-derpem-responses)
...
OpenBao PKI supports the following ACME directories, providing different
...
These endpoints are unauthenticated from an OpenBao authentication model, but
...
an authenticated OpenBao client will need to [fetch a new EAB
...
to be specified by [mount tuning](../system/mounts.mdx#tune-mount-configuration):
...
This endpoint can be used both when signing an OpenBao-backed intermediate or
...
This endpoint returns a list of available CEL roles. Only the role names are
returned, not any values. It is useful to both operators and users.
...
This endpoint queries the CEL role definition. It is useful to both operators and
users.

| Method | Path               |
| :----- | :----------------- |
| `GET`  | `/pki/cel/roles/:name` |
...
### Generate certificate and key against CEL role
...
| Method | Path                                  | Issuer        |
| :----- | :------------------------------------ | :------------ |
| `POST` | `/pki/cel/issue/:name`                | Role selected |
...
- Any arbitrary field can be provided and will be used if recognized in the CEL
  program.

:::info

**Note** Every key/value pair in the request body is forwarded to the CEL
**request** object. A CEL program may decide which of these parameters to
honour, ignore, or override.
...
### Sign certificate against CEL role
...
| Method | Path                                 | Issuer        |
| :----- | :----------------------------------- | :------------ |
| `POST` | `/pki/cel/sign/:name`                    | Role selected |
...
This is an alternative option to the standard method of revoking using OpenBao
...
| `GET`  | `/pki/ca`                      | `default` | [DER](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") |
| `GET`  | `/pki/ca/pem`                  | `default` | [PEM](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") |
| `GET`  | `/pki/issuer/:issuer_ref/json` | Selected  | JSON                                                                           |
| `GET`  | `/pki/issuer/:issuer_ref/der`  | Selected  | [DER](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") |
| `GET`  | `/pki/issuer/:issuer_ref/pem`  | Selected  | [PEM](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") |
...
| Method | Path                 | Issuer    | Format                                                                         |
| :----- | :------------------- | :-------- |:-------------------------------------------------------------------------------|
| `GET`  | `/pki/ca_chain`      | `default` | [PEM](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") |
...
Endpoints with source `local` only include cluster-local revocations.
...
| `GET`  | `/pki/crl`                                      | `default` | [DER](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") | Complete | Local  |
| `GET`  | `/pki/crl/pem`                                  | `default` | [PEM](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") | Complete | Local  |
| `GET`  | `/pki/cert/delta-crl`                           | `default` | JSON                                                                           | Delta    | Local  |
| `GET`  | `/pki/crl/delta`                                | `default` | [DER](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") | Delta    | Local  |
| `GET`  | `/pki/crl/delta/pem`                            | `default` | [PEM](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") | Delta    | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl`                   | Selected  | JSON                                                                           | Complete | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl/der`               | Selected  | [DER](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") | Complete | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl/pem`               | Selected  | [PEM](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") | Complete | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl/delta`             | Selected  | JSON                                                                           | Delta    | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl/delta/der`         | Selected  | [DER](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") | Delta    | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl/delta/pem`         | Selected  | [PEM](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") | Delta    | Local  |
...
At this time there are certain limitations of the OCSP implementation at this path:
 ...
 1. Note that this API will not work with the OpenBao client as both request and responses are DER encoded, and
...
| :----- | :--------------------------------------------------------- | :----------------------------------------------------------------------------- | :------ |
| `GET`  | `/pki/ocsp/<base 64+URL encoded ocsp DER request>`         | [DER](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") | Local   |
| `POST` | `/pki/ocsp`                                                | [DER](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") | Local   |
...
| `GET`  | `/pki/cert/:serial/raw`     | [DER](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") |
| `GET`  | `/pki/cert/:serial/raw/pem` | [PEM](#openbao-cli-with-derpem-responses "OpenBao CLI With DER/PEM Responses") |
...
[imported](#import-ca-certificates-and-keys) into this mount.
...
Note: OpenBao will deduplicate differently-encoded but same-valued keys and
...
**Note**: OpenBao does not allow reading the value of the private key after
it has been created.
...
**Note**: Because OpenBao does not allow exporting the private key
...
As a result, OpenBao prohibits deletion of keys until **all** issuers
...
Refer to the [earlier section](#list-cel-roles) for more information about
listing CEL roles.

### Create/Update CEL role {#create-update-cel-role}

This endpoint creates or updates the CEL role definition.

| Method  | Path               |
| :------ | :----------------- |
| `POST`  | `/pki/cel/roles/:name` |
| `PATCH` | `/pki/cel/roles/:name` |
...
Refer to the [earlier section](#read-cel-role) for more information about
reading roles.

### Delete CEL role

This endpoint deletes the CEL role definition. Deleting a role **does not**
revoke certificates previously issued under this role.

| Method   | Path               |
| :------- | :----------------- |
| `DELETE` | `/pki/cel/roles/:name` |
...
- `allow_ip_sans` `(bool: true)` - Specifies if clients can request IP Subject
  Alternative Names. Allowed IP ranges can be further restricted if
  allowed_ip_sans_cidr is set.

- `allowed_ip_sans_cidr` `(string: "")` - Specifies the IP CIDRs this role is
  allowed to issue certificates for. This is used with the allow_ip_sans to
...
- `no_store` `(bool: false)` - If set, certificates issued/signed against this
  ...
  still be revoked via [BYOC revocation](#revoke-certificate).
...
- `not_before_bound` `(string: "permit")` - Specifies if and how `not_before` can be
  provided when requesting a certificate. Defaults to `permit`. Valid options are :

    - `permit` to allow providing `not_before`.
    - `duration` to allow providing `not_before` within the bounds of `not_before_duration`.
    - `forbid` to disallow providing `not_before`.
...
- `not_after_bound` `(string: "permit")` - Specifies if and how `not_after`
  can be provided when requesting a certificate. Defaults to `permit`. Valid
  options are:

    - `permit` to allow providing `not_after`.
...
a particular cluster. This is useful for populating `{{cluster_path}}` during
AIA URL templating, but may be used for other values in the future.

It also has `aia_path`, which allows using a non-OpenBao, external responder,
...
The cluster-local config has `path`, which sets the URL to this mount on a
particular cluster. This is useful for populating `{{cluster_path}}` during AIA
...
- `path` `(string: "")` - Specifies the path to this cluster's API mount path,
  including any namespaces as path components. For example,
  `https://a.openbao.example.com/v1/ns1/pki-root`.

- `aia_path` `(string: "")` - Specifies the path to this  cluster's AIA
  distribution point; may refer to an external, non-OpenBao responder.  This is
  for resolving AIA URLs and providing the `{{cluster_aia_path}}` template
  parameter and will not be used for other purposes. As such, unlike `path`
...
- `ocsp_disable` `(bool: false)` - Disables or enables the OCSP responder in OpenBao.
...
**Note**: Mirroring the behavior of earlier OpenBao versions, we add
...
- `tidy_revoked_certs` `(bool: false)` - Set to true to remove all revoked and expired
  certificates from storage. A revoked storage entry is considered invalid if the entry
  is empty, or the value within the entry is empty. If a certificate is removed due to
  expiry, the entry will also be removed from the CRL, and the CRL will be rotated.

- `tidy_invalid_certs` `(bool: false)` - Set to true to remove all invalid certificates
  from storage. A certificate is considered invalid if it cannot be parsed by Go's X.509
  certificate parser.
