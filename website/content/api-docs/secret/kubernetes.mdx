...
description: This is the API documentation for the OpenBao Kubernetes secrets engine.
---
...
This is the API documentation for the OpenBao Kubernetes secrets engine. To
...
`/kubernetes` path in OpenBao. Since it is possible to enable secrets engines at
...
- `allowed_kubernetes_namespace_selector` `(string: "")` - A label selector for Kubernetes
  ...
  as illustrated in [Sample Payload 4](kubernetes.mdx#sample-payload-4) and
  [Sample Payload 5](kubernetes.mdx#sample-payload-5) below.
...
- `generated_role_rules` `(string: "")` - The Role or ClusterRole rules to use
  ...
  and [Sample Payload 3](kubernetes.mdx#sample-payload-3) below.
- `name_template` `(string: "")` - The name template to use when generating
  ...
  used. See [username templating](/docs/concepts/username-templating)
...
defined in the selector, or is named `openbao-system` or `testing`.
...
- `cluster_role_binding` `(bool: false)` - If true, generate a ClusterRoleBinding
  ...
  Requires the OpenBao role to have `kubernetes_role_type` set to ClusterRole.
...
- `audiences` `(string: "")` - A comma separated string containing the intended audiences
  ...
  the [token_default_audiences](kubernetes.mdx#token_default_audiences) will be used.
