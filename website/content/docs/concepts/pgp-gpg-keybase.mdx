...
sidebar_label: PGP, GnuPG and Keybase
description: |-
  OpenBao has the ability to integrate with OpenPGP-compatible programs like
...
OpenBao has the ability to integrate with OpenPGP-compatible programs like GnuPG
...
Once the OpenBao has been initialized, it is possible to use Keybase to decrypt
the shards and unseal normally.

## Initializing with PGP

One of the early fundamental problems when bootstrapping and initializing OpenBao
...
To generate unseal keys for Keybase users, OpenBao accepts the `keybase:` prefix
...
Keybase handles a few of the tedious steps. The output will be similar to
the following:
...
decrypt the value. The keys are encrypted in the order specified in
...
can decrypt the value. The keys are encrypted in the order specified
