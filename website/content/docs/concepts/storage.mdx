...
As described on our [Architecture](../internals/architecture.mdx) page, OpenBao's
...
[Storage Configuration](../configuration/storage/index.mdx) section for more
information.

:::info

**Choosing a storage backend:** Refer to the [integrated Storage vs. external
storage](../configuration/storage/index.mdx#integrated-storage-vs-external-storage)
...
mode](../internals/high-availability.mdx). An  OpenBao cluster can extend
across multiple availability zones within a region.

When using OpenBao in [HA Mode](../internals/high-availability.mdx), a
...
Storage](../commands/operator/raft.mdx#snapshot)).
...
- Integrated Storage [snapshots](../commands/operator/raft.mdx#snapshot)
