...
**Note:** Password policies are unrelated to [Policies](policies.mdx) other than sharing similar names.

The Password policies [API docs can be found here](/api-docs/system/policies-password).

:::danger

Password policies are an advanced usage of OpenBao. This generates credentials for external systems
...
[![OpenBao Password Policy Flow](/img/openbao-password-policy-flow.svg)](/img/openbao-password-policy-flow.svg)
...
Where the number of times a candidate password needs to be generated is a function of the likelihood that a given
...
[![Password Policy Performance](/img/openbao-password-policy-performance.svg)](/img/openbao-password-policy-performance.svg)
...
being selected from a subset are smaller. For instance, if you have a 1 character password from the charset
...
[![Chance of Generating a Good Password - 1](/img/openbao-password-policy-chance.svg)](/img/openbao-password-policy-chance.svg)
...
OpenBao ships with a default password policy that applies to any password
