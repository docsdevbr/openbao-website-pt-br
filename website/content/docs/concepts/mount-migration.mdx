...
OpenBao supports moving secrets and auth mounts, both within and across namespaces. The operation
is supported by API endpoints and a CLI helper around them.

## Why

There can be several reasons for wanting to migrate mounts. In OpenBao, the use cases could be for
...
will start the migration process, and the API will return a migration ID. This ID, in turn, will be used to monitor
...
There are two primary cleanup operations that OpenBao operators will need to keep in mind after performing a remount.
