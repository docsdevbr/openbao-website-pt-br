...
description: Tokens are a core auth method in OpenBao. Concepts and important features.
---

# Tokens

:::warning[Internal token structure is volatile]

  Tokens are opaque values so their structure is undocumented and subject to change.
  Scripts and automations that rely on the internal structure of a token will break.

:::

Tokens are the core method for _authentication_ within OpenBao. Tokens
can be used directly or [auth methods](auth.mdx)
...
`bao server -dev` (or `bao operator init` for a non-dev server) outputs an
...
As stated in the [authentication concepts](auth.mdx),
...
Within OpenBao, tokens map to information. The most important information mapped
to a token is a set of one or more attached
[policies](policies.mdx). These policies control what the token
...
backend](../auth/token.mdx). This is a special
...
tokens can do anything in OpenBao. _Anything_. In addition, they are the only
...
1. The initial root token generated at `bao operator init` time -- this token has no
   expiration
1. By using another root token; a root token with an expiration cannot create a
   root token that never expires
1. By using `bao operator generate-root`
...
If the token is renewable, OpenBao can be asked to extend the token validity
period using `bao token renew` or the appropriate renewal endpoint. At this
...
1. The system max TTL, which is 32 days but can be changed in OpenBao's
   configuration file.
...
Service tokens are what users will generally think of as "normal" OpenBao tokens.
