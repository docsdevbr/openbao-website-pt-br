...
OpenBao supports a number of storage options for the durable storage of OpenBao's
...
The option stores OpenBao's data on the server's filesystem and
...
documentation](../../internals/integrated-storage.mdx). Additionally, the
[Configuration](../../configuration/storage/raft.mdx) docs can help in configuring
OpenBao to use Integrated Storage.

The sections below go into various details on how to operate OpenBao with
Integrated Storage.
...
[`cluster_addr`](../ha.mdx#per-node-cluster-address) configuration option
is set. This allows OpenBao to assign an address to the node ID at join time.

## Cluster membership

This section will outline how to bootstrap and manage a cluster of OpenBao nodes
running Integrated Storage.

Integrated Storage is bootstrapped during the initialization process
and results in a cluster of size 1. Depending on the [desired deployment
size](../../internals/integrated-storage.mdx#tabela-de-implantação), nodes can be joined
to the active OpenBao node.

### Joining nodes

Joining is the process of taking an uninitialized OpenBao node and making it a
...
recommend setting the [`api_addr`](../ha.mdx#direct-access) configuration
...
leaders becomes active this node will successfully join. When using Shamir seal,
...
An example [`retry_join`](../../configuration/storage/raft.mdx#retry_join-stanza)
...
Note, in each [`retry_join`](../../configuration/storage/raft.mdx#retry_join-stanza)
stanza, you may provide a single
[`leader_api_addr`](../../configuration/storage/raft.mdx#leader_api_addr) or
[`auto_join`](../../configuration/storage/raft.mdx#auto_join) value. When a cloud
[`auto_join`](../../configuration/storage/raft.mdx#auto_join) configuration value is
provided, OpenBao will use [go-discover](https://github.com/hashicorp/go-discover)
...
details on the format of the [`auto_join`](../../configuration/storage/raft.mdx#auto_join) value.
...
[`auto_join_scheme`](../../configuration/storage/raft.mdx#auto_join_scheme) and
[`auto_join_port`](../../configuration/storage/raft.mdx#auto_join_port) fields
respectively.
...
command](../../commands/operator/raft.mdx#join) or the API to join a node. The
...
[`remove-peer`](../../commands/operator/raft.mdx#remove-peer) command and provide the
...
[`list-peers`](../../commands/operator/raft.mdx#list-peers) command. All the voting
...
Before we explore solutions to this problem, let's recapitulate how OpenBao nodes
speak to one another.

OpenBao exposes two TCP ports: [the API port](../../configuration/index.mdx#api_addr) and
[the cluster port](../../configuration/index.mdx#cluster_addr).
...
When you have multiple nodes, you also need a cluster port. This is used by OpenBao
...
The cluster port is secured using a TLS certificate that the OpenBao active node
...
The simplest option is to do it by hand: issue [`raft join`](../../commands/operator/raft.mdx#join) commands specifying the explicit names
...
in the [`retry_join`](../../configuration/storage/raft.mdx#retry_join-stanza)
...
[`retry_join`](../../configuration/storage/raft.mdx#retry_join-stanza) with the LB
...
issue a [`remove-peer`](../../commands/operator/raft.mdx#remove-peer) command to
remove the failed server if it's still a member of the cluster.

If the [`remove-peer`](../../commands/operator/raft.mdx#remove-peer) command isn't
possible or you'd rather manually re-write the cluster membership a
[`raft/peers.json`](#manual-recovery-using-peersjson) file can be written to the
...
[`peers.json`](#manual-recovery-using-peersjson) for details of the recovery
procedure. You include only the remaining servers in the
[`peers.json`](#manual-recovery-using-peersjson) recovery file. The cluster
should be able to elect a leader once the remaining servers are all restarted
with an identical [`peers.json`](#manual-recovery-using-peersjson)
configuration.
...
[`peers.json`](#manual-recovery-using-peersjson) recovery file.
...
path](../../configuration/storage/raft.mdx#path) of each OpenBao server. Inside that
...
For other, non-quorum related recovery [OpenBao's
recovery](../recovery-mode.mdx) mode can be used.
