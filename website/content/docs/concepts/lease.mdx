...
description: >-
  OpenBao provides a lease with every secret. When this lease is expired, OpenBao
  will revoke that secret.
---

# Lease, renew, and revoke

With every dynamic secret and `service` type authentication token, OpenBao
...
All dynamic secrets in OpenBao are required to have a lease. Even if the data is
...
example, with the [Kubernetes secrets engine](../secrets/kubernetes.mdx), the
...
Revocation can happen manually via the API, via the `bao lease revoke` cli command,
...
**Note**: The [Key/Value Backend](../secrets/kv/index.mdx) which stores
...
When reading a dynamic secret, such as via `bao read`, OpenBao always returns a
`lease_id`. This is the ID used with commands such as `bao lease renew` and `bao lease revoke` to manage the lease of the secret.
...
example, `bao lease renew -increment=3600 my-lease-id` would request that the TTL of the lease
...
To implement token renewal logic in your application code, refer to the [code example in the Authentication doc](auth.mdx#code-example).
...
example, to revoke all Userpass logins, you can do `bao lease revoke -prefix auth/userpass/`.
For more information about revoke command please check
[cli's lease revoke](../commands/lease/revoke.mdx#lease-revoke)
command docs.
