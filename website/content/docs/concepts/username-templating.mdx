...
Some of the secrets engines that generate dynamic users for external systems provide the ability for OpenBao operators
...
[Database engine's](../secrets/databases/index.mdx) data, however the specific fields that are provided from a given engine
