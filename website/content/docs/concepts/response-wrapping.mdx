...
In many OpenBao deployments, clients can access OpenBao directly and consume
...
To help address this problem, OpenBao includes a feature called _response
wrapping_. When requested, OpenBao can take the response it would have sent to an
HTTP client and instead insert it into the
[`cubbyhole`](../secrets/cubbyhole.mdx) of a single-use token,
...
OpenBao currently does not provide signed response-wrapping tokens, as it
...
Response wrapping is per-request and is triggered by providing to OpenBao the
...
(`25h`). When using the OpenBao CLI, you can set this via the `-wrap-ttl`
parameter. When using the Go API, wrapping is triggered by [setting a helper
function](https://godoc.org/github.com/openbao/openbao/api#Client.SetWrappingLookupFunc)
