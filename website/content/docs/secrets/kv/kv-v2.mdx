...
Once upgraded to version 2, the former paths at which the data was accessible will no longer suffice. You will need to adjust user policies to add access to the version 2 paths as detailed in the [ACL Rules section below](kv-v2.mdx#acl-rules). Similarly, users/applications will need to update the paths at which they interact with the kv data once it has been upgraded to version 2.
...
not supported for policies used with the version 2 kv store. See the [Policies Concepts](../../concepts/policies.mdx)
...
1. Write arbitrary data:
   ...
   ```shell-session
   $ bao kv put -mount=secret my-secret foo=a bar=b
   Key              Value
   ---              -----
...
1. Write another version, the previous version will still be accessible. The
  ...
  current version.
   ...
   ```shell-session
   $ bao kv put -mount=secret -cas=1 my-secret foo=aa bar=bb
   Key              Value
   ---              -----
...
1. Partial updates can be accomplished using the `bao kv patch` command. A
   ...
   ```shell-session
   $ bao kv patch -mount=secret -cas=2 my-secret bar=bbb
   Key              Value
   ---              -----
...
1. The `bao kv patch` command also supports a `-method` flag which can be
   ...
   ```shell-session
   $ bao kv patch -mount=secret -method=patch -cas=2 my-secret bar=bbb
   Key              Value
   ---              -----
   created_time     2019-06-19T17:23:49.199802Z
   custom_metadata  <nil>
   ...
   ```shell-session
   $ bao kv patch -mount=secret -method=rw my-secret bar=bbb
   Key              Value
   ---              -----
   created_time     2019-06-19T17:23:49.199802Z
   custom_metadata  <nil>
...
3. Previous versions can be accessed with the `-version` flag:
   ...
   ```shell-session
   $ bao kv get -mount=secret -version=1 my-secret
   ====== Metadata ======
   Key              Value
   ---              -----
   created_time     2019-06-19T17:20:22.985303Z
...
You can also use [OpenBao's password policy](../../concepts/password-policies.mdx) feature to generate arbitrary values.
...
1. Write data using the `example` policy:
   ...
   ```shell-session
   $ bao kv put -mount=secret my-generated-secret \
...
1. Read the generated data:
   ...
   ```shell-session
   $ bao kv get -mount=secret my-generated-secret
   ========= Secret Path =========
   secret/data/my-generated-secret
...
When deleting data the standard `bao kv delete` command will perform a
...
which allows the version to be undeleted. The `bao kv undelete` command
handles undeleting versions.

A version's data is permanently deleted only when the key has more versions than
are allowed by the max-versions setting, or when using `bao kv destroy`. When
...
1. All metadata and versions for a key can be viewed:
   ...
   ```shell-session
   $ bao kv metadata get -mount=secret my-secret
   ========== Metadata ==========
   Key                     Value
   ---                     -----
   cas_required            false
...
1. The metadata settings for a key can be configured:
   ...
   ```shell-session
   $ bao kv metadata put -mount=secret -max-versions 2 -delete-version-after="3h25m19s" my-secret
   ...
   ```shell-session
   $ bao kv put -mount=secret my-secret my-value=newer-s3cr3t
   Key              Value
   ---              -----
   ...
   The `bao kv metadata put` command can be used to fully overwrite the value of `custom_metadata`:

   ```shell-session
   $ bao kv metadata put -mount=secret -custom-metadata=foo=abc -custom-metadata=bar=123 my-secret
   Success! Data written to: secret/metadata/my-secret
   ...
   The `bao kv metadata patch` command can be used to partially overwrite the value of `custom_metadata`.
   The following invocation will update `custom_metadata` sub-field `foo` but leave `bar` untouched:

   ```shell-session
   $ bao kv metadata patch -mount=secret -custom-metadata=foo=def my-secret
   Success! Data written to: secret/metadata/my-secret
   ```
...
1. Permanently delete all metadata and versions for a key:
   ...
   ```shell-session
   $ bao kv metadata delete -mount=secret my-secret
