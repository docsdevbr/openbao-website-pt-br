...
description: Secrets engines are mountable engines that store or generate secrets in OpenBao.
---
...
Some secrets engines simply store and read data - like an encrypted version of
Valkey/Memcached. Other secrets engines connect to other services and generate
...
Secrets engines are enabled at a **path** in OpenBao. When a request comes to
...
- [Enable](../commands/secrets/enable.mdx) - This enables a secrets engine at
...
- [Disable](../commands/secrets/disable.mdx) - This disables an existing
...
- [Move](../commands/secrets/move.mdx) - This moves the path for an existing
...
- [Tune](../commands/secrets/tune.mdx) - This tunes global configuration for
  the secrets engine such as the TTLs.

Once a secrets engine is enabled, you can interact with it directly at its path
according to its own API. Use `bao path-help` to determine the paths it
responds to.

Note that mount points cannot conflict with each other in OpenBao. There are
...
Secrets engines receive a _barrier view_ to the configured OpenBao physical
...
This is an important security feature in OpenBao - even a malicious engine
