...
sidebar_label: SSH OTP
description: |-
  The One-Time SSH Password (OTP) SSH secrets engine type allows an OpenBao server
...
The One-Time SSH Password (OTP) SSH secrets engine type allows an OpenBao server to
...
An authenticated client requests credentials from the OpenBao server and, if
...
Since the OpenBao server is contacted during SSH connection establishment, every
