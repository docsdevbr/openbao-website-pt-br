...
sidebar_label: Signed Certificate
description: >-
...
Before a client can request their SSH key be signed, the OpenBao SSH secrets engine must
...
The following steps are performed in advance by an OpenBao administrator, security
team, or configuration management tooling.

1.  Mount the secrets engine. Like all secrets engines in OpenBao, the SSH secrets engine
...
1.  Configure OpenBao with a CA for signing client keys using the `/config/ca`
...
1.  Ask OpenBao to sign your **public key**. This file usually ends in `.pub` and
...
1.  SSH into the host machine using the signed key. You must supply both the
    signed public key from OpenBao **and** the corresponding private key as
...
1.  Configure OpenBao with a CA for signing host keys using the `/config/ca`
    endpoint. If you do not have an internal CA, OpenBao can generate a keypair for
    you.
...
**IMPORTANT:** Do NOT add a private key password since OpenBao can't decrypt it.
