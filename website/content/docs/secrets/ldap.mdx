...
- [Static Credentials](ldap.mdx#static-credentials)
- [Dynamic Credentials](ldap.mdx#dynamic-credentials)
- [Service Account Check-Out](ldap.mdx#service-account-check-out)
...
password can be configured using a [password policy](../concepts/password-policies.mdx):
...
if it's renewed. These default to `24h`, and both use [duration format strings](../concepts/duration-format.mdx).
