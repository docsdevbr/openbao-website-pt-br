...
description: >-
  The RabbitMQ secrets engine for OpenBao generates user credentials to access
  RabbitMQ.
---
...
OpenBao makes use both of its own internal revocation system as well as the
...
1.  Configure the credentials that OpenBao uses to communicate with RabbitMQ to
    ...
    It is important that the OpenBao user have the administrator privilege to
    manager users.

1.  Configure a role that maps a name in OpenBao to virtual host permissions:
