...
This document provides a brief overview of setting up an OpenBao PKI Secrets
...
Next, OpenBao must be configured with a CA certificate and associated private
...
```shell-session
$ bao write pki/root/generate/internal common_name=myopenbao.com ttl=87600h
Key             Value
---             -----
...
```shell-session
$ bao write pki/config/urls issuing_certificates="http://openbao.example.com:8200/v1/pki/ca" crl_distribution_points="http://openbao.example.com:8200/v1/pki/crl"
...
```shell-session
$ bao write pki/issue/example-dot-com \
...
If you get stuck at any time, simply run `bao path-help pki` or with a
