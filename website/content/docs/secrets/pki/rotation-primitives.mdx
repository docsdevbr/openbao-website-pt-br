...
sidebar_label: Rotation Primitives
description: The PKI secrets engine for OpenBao generates TLS certificates.
---

# PKI secrets engine - rotation primitives

OpenBao's PKI Secrets Engine supports multiple issuers in a single mount point.
...
The following fields on the final certificate are relevant to rotation:
 ...
 - The [Validity](https://datatracker.ietf.org/doc/html/rfc5280#section-4.1.2.5)
   ...
   - Notably, RFC 5280 does not place any requirements around the issued
     ...
     up to their notAfter date. This is why OpenBao's `/pki/issuer/:issuer_ref`
...
types that use the [above certificate fields](#x509-certificate-fields).
...
To create a cross-signed certificate in OpenBao, use the [`/intermediate/cross-sign`
...
To create a reissued root certificate in OpenBao, use [`/issuers/generate/root/existing`
...
To create a reissued intermediate certificate in OpenBao, this is a three step
process:
...
practices](considerations.mdx#use-a-ca-hierarchy) are
...
3. Encourage rotation to pickup the new cross-signed intermediates. With
   ...
   automatically](considerations.mdx#automate-leaf-certificate-renewal).
