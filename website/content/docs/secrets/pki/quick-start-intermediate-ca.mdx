...
sidebar_label: Quick Start - Intermediate CA Setup
...
In the [first Quick Start guide](quick-start-root-ca.mdx),
...
To add another certificate authority to our OpenBao instance, we have to mount it
at a different path.
...
```shell-session
$ bao write pki_int/intermediate/generate/internal common_name="myopenbao.com Intermediate Authority" ttl=43800h
Key Value
...
```shell-session
$ bao write pki/root/sign-intermediate csr=@pki_int.csr format=pem_bundle ttl=43800h
Key             Value
...
```shell-session
$ bao write pki_int/config/urls issuing_certificates="http://127.0.0.1:8200/v1/pki_int/ca" crl_distribution_points="http://127.0.0.1:8200/v1/pki_int/crl"
...
```shell-session
$ bao write pki_int/roles/example-dot-com \
...
to the `issue` endpoint with that role name: OpenBao is now configured to create
and manage certificates!

```shell-session
$ bao write pki_int/issue/example-dot-com \
...
OpenBao has now generated a new set of credentials using the `example-dot-com`
