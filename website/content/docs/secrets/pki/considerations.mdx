...
sections](rotation-primitives.mdx), this should keep the
...
automatically renewing requested certificates](../../agent-and-proxy/agent/template.mdx#certificates)
...
Note: Depending on usage of OpenBao, CRLs (and rarely, CA chains) can grow to
   ...
   If this is done with only a [syslog](../../audit/syslog.mdx) audit device,
   ...
   response fields HMACed and/or to also enable the [`file`](../../audit/file.mdx)
   audit log type.
