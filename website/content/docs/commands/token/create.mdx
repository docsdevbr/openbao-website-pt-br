...
- `-explicit-max-ttl` `(duration: "")` - Explicit maximum lifetime for the
  ...
  exceeded. Uses [duration format strings](../../concepts/duration-format.mdx).
...
- `-period` `(duration: "")` - If specified, every renewal will use the given
  ...
  requires sudo permissions. Uses [duration format strings](../../concepts/duration-format.mdx).
...
- `-ttl` `(duration: "")` - Initial TTL to associate with the token. Token
  ...
  maximumTTLs. Uses [duration format strings](../../concepts/duration-format.mdx).
