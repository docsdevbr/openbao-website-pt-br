...
description: |-
  The "login" command authenticates users or machines to OpenBao using the
...
The `login` command authenticates users or machines to OpenBao using the provided
...
```shell-session
$ bao login s.3jnbMAKl1i4YS3QoKdbHzGXq
Success! You are now authenticated. The token information displayed below
is already stored in the token helper. You do NOT need to run "bao login"
again. Future OpenBao requests will automatically use this token.
...
- `-path` `(string: "")` - Remote path in OpenBao where the auth method
