...
- `options` - [Flags](index.mdx#flags) to specify additional settings
...
The four most common operations in OpenBao are [read](read.mdx),
[write](write.mdx), [delete](delete.mdx), and
[list](list.mdx). These operations work on most paths in OpenBao. Some
...
To invoke the OpenBao API paths, you can use the [read](read.mdx) (for
HTTP GET), [write](write.mdx) (for HTTP PUT or POST),
[delete](delete.mdx) (for HTTP DELETE), and
[list](list.mdx) (for HTTP LIST) commands.
