...
```shell-session
$ bao auth tune -default-lease-ttl=-1 github/
Success! Tuned the auth method at: github/
```
...
```shell-session
$ bao auth tune -audit-non-hmac-request-keys=value1 -audit-non-hmac-request-keys=value2 github/
Success! Tuned the auth method at: github/
```
...
[userpass](../../auth/userpass.mdx), [ldap](../../auth/ldap.mdx), and
[approle](../../auth/approle.mdx) auth methods.

Tune the `userpass/` auth method to lock out the user after 10 failed login
attempts within 10 minutes.

```shell-session
$ bao auth tune -user-lockout-threshold=10  -user-lockout-duration=10m userpass/
Success! Tuned the auth method at: userpass/
```

View the current configuration of the auth method enabled at `userpass/`.

```shell-session {1,11-13}
...
- `-max-lease-ttl` `(duration: "")` - The maximum lease TTL for this auth
  method. If unspecified, this defaults to the OpenBao server's globally
  configured [maximum lease TTL](../../configuration/index.mdx#max_lease_ttl), or a
