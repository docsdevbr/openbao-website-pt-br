...
concepts](../../concepts/policies.mdx) page for more information.
...
```shell-session
$ bao write auth/userpass/users/sethvargo password=secret
...
the [auth method](../../auth/index.mdx) documentation.
...
- `-audit-non-hmac-request-keys` `(string: "")` - Key that will not be HMAC'd
  ...
  An example of this is provided in the [tune section](tune.mdx).
...
- `-default-lease-ttl` `(duration: "")` - The default lease TTL for this auth
  ...
  method. Uses [duration format strings](../../concepts/duration-format.mdx).
