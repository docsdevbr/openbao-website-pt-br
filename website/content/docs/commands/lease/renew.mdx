...
description: |-
  ...
  that it can be used before it is revoked by OpenBao.
---

# lease renew

The `lease renew` command renews the lease on a secret, extending the time that
it can be used before it is revoked by OpenBao.

Every secret in OpenBao has a lease associated with it. If the owner of the secret
...
```shell-session
$ bao lease renew database/creds/readonly/27e1b9a1-27b8-83d9-9fe0-d99d786bdc83
Key                Value
---                -----
...
- `-increment` `(duration: "")` - Request a specific increment in seconds. OpenBao
