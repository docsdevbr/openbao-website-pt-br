...
description: |-
  The "plugin register" command registers a new plugin in OpenBao's plugin
  catalog.
---

# plugin register

The `plugin register` command registers a new plugin in OpenBao's plugin catalog.
...
- `-oci` `(bool: false)` - Whether the plugin is a declaratively donwloaded OCI plugin.
  Note that manually registering such plugins means that `plugin_auto_register=true`
  cannot be used.
