...
description: |-
  The "patch" command updates data in OpenBao at the given path. The data can be
...
This causes `bao patch` to read a JSON blob containing all request parameters
...
Unlike [the `write` command](write.mdx), the `patch` command only
