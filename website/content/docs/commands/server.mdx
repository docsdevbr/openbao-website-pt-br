...
- [`operator init` command](operator/init.mdx) for information
  on initializing an OpenBao server.

- [`operator unseal` command](operator/unseal.mdx) for
  information on providing unseal keys.

- [OpenBao configuration](../configuration/index.mdx) for the syntax and
...
- `-config` `(string: "")` - Path to a configuration file or directory of
  ...
  The environment variable `BAO_CONFIG_PATH` may also be used to specify multiple
  configuration paths using a comma (,) as a separator.
