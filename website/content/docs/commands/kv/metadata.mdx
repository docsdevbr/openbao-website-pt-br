...
description: |-
  ...
  endpoint in OpenBao's key-value store.
---
...
```shell-session
$ bao kv metadata delete -mount=secret creds
...
```shell-session
$ bao kv metadata put -mount=secret creds
...
```shell-session
$ bao kv metadata put -mount=secret -max-versions=5 creds
Success! Data written to: secret/metadata/creds
```
...
- `-delete-version-after` `(string:"0s")` – Set the `delete-version-after` value
  ...
  `delete_version_after` will be used. Accepts [duration format strings](../../concepts/duration-format.mdx).
