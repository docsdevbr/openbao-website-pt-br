...
```shell-session
$ bao secrets tune -default-lease-ttl=18h -audit-non-hmac-request-keys=common_name -audit-non-hmac-response-keys=serial_number pki/
...
- `-default-lease-ttl` `(duration: "")` - The default lease TTL for this secrets
  ...
  engine. Uses [duration format strings](../../concepts/duration-format.mdx).
