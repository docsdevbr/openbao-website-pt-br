...
the mount and `/sys` against the given OpenBao server
...
1. Perform a [CA rotation operation](../../secrets/pki/rotation-primitives.mdx)
...
1. Delete any expired CAs with one of the following options:
  - Run [tidy](/api-docs/secret/pki#tidy) manually with `bao write <mount>/tidy tidy_expired_issuers=true`.
  - Use the OpenBao API to call [delete issuer](/api-docs/secret/pki#delete-issuer).
...
1. Use [BYOC revocations](/api-docs/secret/pki#revoke-certificate) to
...
1. Update `allowed_response_headers` and `passthrough_request_headers` for all
   policies with `bao secrets tune`:
...
1. Review the tidy status endpoint, `bao read <mount>/tidy-status` for
...
Review the [ACME Certificate Issuance](/api-docs/secret/pki#acme-certificate-issuance)
