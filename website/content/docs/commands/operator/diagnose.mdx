...
description: |-
  "bao operator diagnose" is a new operator-centric command, focused on providing a clear description
...
`OpenBao Diagnose` is the top level check that contains the rest of the checks. It will report the status
of the check

#### Check operating system / check open file limit

`Check Open File Limit` verifies that the open file limit value is set high enough for OpenBao
...
`Parse Configuration` will check the OpenBao server config file for syntax errors. It will check
...
`Create Storage Backend` ensures that the storage stanza configured in the OpenBao server config
...
`Check Consul Direct Service Discovery` is a consul-specific check that ensures OpenBao
is not accessing the consul server directly, but rather through a local agent.

#### Create OpenBao server configuration seals

`Create OpenBao Server Configuration Seals` creates seals from the OpenBao configuration
...
`Initialize Randomness for Core` ensures that OpenBao has access to the randReader that
...
`ha_storage` section of the OpenBao configuration, as opposed to the `storage` section.
...
`Check Core Creation` verifies the logical configuration checks that OpenBao does when it
