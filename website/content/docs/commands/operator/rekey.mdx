...
Note: `operator rekey` command is deprecated, please use [rotate-keys](rotate-keys.mdx) instead.
...
- `-init` `(bool: false)` - Initialize the rekeying operation. This can only be
  done if there's not one in progress. Customize the new number of key shares and
...
- `-target` `(string: "barrier")` - Target for rekeying. "recovery" only applies
  when HSM support is enabled or using [Auto Unseal](../../concepts/seal.mdx#auto-unseal).
