...
* Ability to deploy a Prometheus Service Monitor, configured via the Helm Chart [`telemetry`](helm.mdx#h-telemetry) stanza.
* Ability to deploy a Metrics Service, configured via the Helm Chart [`metricsService`](helm.mdx#h-metricsservice) stanza.
