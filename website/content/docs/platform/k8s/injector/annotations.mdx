...
- `openbao.openbao.org/template-static-secret-render-interval` - If specified,
  ...
  See [OpenBao Agent Template Config](../../../agent-and-proxy/agent/template.mdx#template-configurations) for more details.
...
- `openbao.openbao.org/agent-cache-enable` - configures OpenBao Agent to enable
  [caching](../../../agent-and-proxy/agent/caching/index.mdx). In OpenBao 1.7+ this annotation will also enable
...
- `openbao.openbao.org/auth-config` - configures additional parameters for the configured
  ...
  see the OpenBao Agent [auto-auth documentation](../../../agent-and-proxy/autoauth/methods/index.mdx).
...
- `openbao.openbao.org/auth-min-backoff` - set the [min_backoff](../../../agent-and-proxy/autoauth/index.mdx#min_backoff) option in the auto-auth config. Requires OpenBao 1.11+.
...
- `openbao.openbao.org/agent-disable-idle-connections` - Comma-separated [list
  of OpenBao Agent features](../../../agent-and-proxy/agent/index.mdx#disable_idle_connections) where idle
...
- `openbao.openbao.org/agent-disable-keep-alives` - Comma-separated [list of
  OpenBao Agent features](../../../agent-and-proxy/agent/index.mdx#disable_keep_alives) where keep-alives
