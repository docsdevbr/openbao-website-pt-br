...
description: This section documents examples of using the OpenBao CSI Provider.
---

# OpenBao CSI provider examples

The following examples demonstrate how the OpenBao CSI Provider can be used.

:::warning

A common mistake is to not install the CSI Secret Store Driver before using the OpenBao CSI Provider.

:::

## File based dynamic database credentials

The following Secret Provider Class retrieves dynamic database credentials from OpenBao and
...
The pod mounts a CSI volume and specifies the Secret Provider Class (`openbao-db-creds`) created above.
...
spec:
  provider: openbao
  secretObjects:
    - secretName: openbao-db-creds-secret
      type: Opaque
      data:
...
secret called `openbao-db-creds` is created and referenced in two environment variables.
