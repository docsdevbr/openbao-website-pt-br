...
description: |-
  The telemetry stanza specifies various configurations for OpenBao to publish
...
The `telemetry` stanza specifies various configurations for OpenBao to publish
metrics to upstream systems. Available OpenBao metrics can be found in the
[Telemetry internals documentation](../internals/telemetry/index.mdx).
...
- `usage_gauge_period` `(string: "10m")` - Specifies the interval at which high-cardinality
  usage data is collected, such as token counts, entity counts, and secret counts.
   A value of "none" disables the collection.  Uses [duration format strings](../concepts/duration-format.mdx).
...
- `lease_metrics_epsilon` `(string: "1h")` - Specifies the size of the bucket used to measure future
  ...
  minutes. Uses [duration format strings](../concepts/duration-format.mdx).
...
- `dogstatsd_addr` `(string: "")` - This provides the address of a DogStatsD
  ...
  OpenBao will send various telemetry information to that instance for
...
OpenBao does not use the default Prometheus path, so Prometheus must be configured
with the path below.
Note that using `?format=prometheus` in the path won't work as "?" will be
escaped, so it must be specified as a parameter.

An OpenBao token is required with `capabilities = ["read", "list"]` to
...
scrape_configs:
  - job_name: 'openbao'
    ...
    bearer_token: "your_openbao_token_here"
    static_configs:
    - targets: ['your_openbao_server_here:8200']
```

An example telemetry configuration to be added to OpenBao's configuration file is shown below:
...
- `stackdriver_debug_logs` `(bool: "false")` - Specifies if OpenBao writes additional stackdriver
...
Metrics from OpenBao can be found in [Metrics Explorer](https://cloud.google.com/monitoring/charts/metrics-explorer).
...
[telemetry-tcp]: listener/tcp.mdx#telemetry-parameters
