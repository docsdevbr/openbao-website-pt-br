...
description: |-
  The AWS KMS seal configures OpenBao to use AWS KMS as the seal wrapping
  mechanism.
---

# `awskms` seal

The AWS KMS seal configures OpenBao to use AWS KMS as the seal wrapping mechanism.
...
This example shows configuring AWS KMS seal through the OpenBao configuration file
...
These can be granted via IAM permissions on the principal that OpenBao uses, on
