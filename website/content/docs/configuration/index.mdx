...
- `user_lockout` <code>([UserLockout][userlockout]: nil)</code>. –
  ...
  [user lockout configuration documentation](user-lockout.mdx).
...
- `plugin` <code>([Plugin][oci-plugins]: &lt;none&gt;)</code> – Specifies declarative plugin
  downloading from OCI images and registration. Can be specified multiple times.

- `plugin_auto_download` `bool: false` - Whether automatic plugin downloading is enabled. See
  [OCI plugin documentation][oci-plugins] for more information.

- `plugin_auto_register` `bool: false` - Whether automatic plugin registration is enabled. See
  [OCI plugin documentation][oci-plugins] for more information.

- `plugin_download_behavior` `string: "fail"` - Whether plugin downloading is treated as fatal
  failure. See [OCI plugin documentation][oci-plugins] for more information.
...
- `max_lease_ttl` `(string: "768h")` – Specifies the maximum possible lease
  ...
  [auth](../commands/auth/tune.mdx#max-lease-ttl) or
  [secret](../commands/secrets/tune.mdx#max-lease-ttl) commands.
...
- `ui` `(bool: false)` – Enables the built-in web UI, which is available on all
  ...
  [ui configuration documentation](ui.mdx).
...
- `log_format` - Equivalent to the [`-log-format` command-line flag](../commands/server.mdx#_log_format).

- `log_file` - Equivalent to the [`-log-file` command-line flag](../commands/server.mdx#_log_file).

- `log_rotate_duration` - Equivalent to the [`-log-rotate-duration` command-line flag](../commands/server.mdx#_log_rotate_duration).

- `log_rotate_bytes` - Equivalent to the [`-log-rotate-bytes` command-line flag](../commands/server.mdx#_log_rotate_bytes).

- `log_rotate_max_files` - Equivalent to the [`-log-rotate-max-files` command-line flag](../commands/server.mdx#_log_rotate_max_files).
...
- `unsafe_allow_api_audit_creation` `(bool: false)` - Allow creating [audit devices](../audit/index.mdx)
...
- `allow_audit_log_prefixing` `(bool: false)` - Allow creating [audit devices](../audit/index.mdx)
  that use the [`prefix` option](../audit/index.mdx#common-configuration-options). Just
...
[audit]: audit.mdx
[storage-backend]: storage/index.mdx
[listener]: listener/index.mdx
[seal]: seal/index.mdx
[telemetry]: telemetry.mdx
[initialize]: self-init.mdx
[high-availability]: ../concepts/ha.mdx
[plugins]: ../plugins/index.mdx
[oci-plugins]: plugins.mdx
[userlockout]: user-lockout.mdx
