...
description: Learn about the security model of OpenBao.
---

# Security model

Due to the nature of OpenBao and the confidentiality of data it manages,
...
- **Eavesdropping on any OpenBao communication.** Client communication with OpenBao
...
- **Confidentiality of stored secrets.** Any data that leaves OpenBao to rest in the
...
- **Protecting against memory analysis of a running OpenBao.** If an attacker is able
  to inspect the memory state of a running OpenBao instance, then the confidentiality
  of data may be compromised.

- **Protecting against flaws in external systems or services used by OpenBao.**
...
- **Protecting against flaws in clients or systems that access OpenBao.** If an attacker
...
- **Protecting against OpenBao administrators supplying vulnerable or malicious configuration
  data.** Any data provided as configuration values to OpenBao's administrative endpoints
  (e.g. [secret engines](../secrets/index.mdx) configurations), or OpenBao's
...
There is no mutual trust between the OpenBao client and server. Clients use
...
All server-to-server traffic between OpenBao instances within a cluster uses
...
[unseal challenge](../concepts/integrated-storage/index.mdx#vault-networking-recap).

The storage backends used by OpenBao are also untrusted by design. OpenBao uses a
security barrier for all requests made to the backend. The security barrier
automatically encrypts all data leaving OpenBao using a 256-bit [Advanced
Encryption Standard
...
Depending on the backend used, OpenBao may communicate with the backend over TLS
...
When a client first authenticates with OpenBao, an auth method is used to verify
...
On each request, a client provides this token. OpenBao then uses it to check that
...
Lastly, OpenBao supports using a [Two-person
rule](https://en.wikipedia.org/wiki/Two-person_rule) for unsealing using [Shamir's
Secret Sharing
technique](https://en.wikipedia.org/wiki/Shamir's_Secret_Sharing). When OpenBao
