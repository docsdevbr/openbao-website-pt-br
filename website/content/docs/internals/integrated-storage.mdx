...
OpenBao's Integrated Storage uses a [consensus
...
and managing when an entry is committed. The leader node is also the active OpenBao node and followers are standby nodes. Refer to the [High Availability](high-availability.mdx#design-overview) document for more information.
...
[BoltDB](https://github.com/etcd-io/bbolt) to maintain a cluster state. OpenBao's writes
...
to run 5 OpenBao servers per cluster. See the [Minimum &
...
When getting started, a single OpenBao server is
[initialized](../commands/operator/init.mdx#operator-init). At this point, the
...
The join process is how new nodes are added to the OpenBao cluster; it uses an
...
An [Autopilot feature](../concepts/integrated-storage/autopilot.mdx)
is available in HashiCorp Vault 1.7.x & later versions that includes configurable parameters
...
Autopilot is enabled by default and the default configuration values
...
accepted as non-voters initially until they are in sync with matching Raft index
and only after reaching a stability threshold are they then full voting members.
...
A dead server cleanup capability is available. With this feature
...
`remove peer` [command](../commands/operator/raft.mdx#remove-peer).
...
The OpenBao Reference Architecture recommends a 5 node cluster
to ensure a minimum failure tolerance of at least 2.
...
The intent with any change or scaling ought to be with the loss of quorum and
