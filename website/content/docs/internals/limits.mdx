...
stanza](../configuration/storage/raft.mdx#max_entry_size).
...
The number of entities can be monitored using OpenBao's [telemetry](telemetry/index.mdx#token-identity-and-lease-metrics); see `vault.identity.num_entities` (total) or `vault.identity.entities.count` (by namespace).
...
Instead, consider using [external groups](../concepts/identity.mdx#external-vs-internal-groups) or split the group up into multiple sub-groups.
...
[`vault.core.fetch_acl_and_token`](telemetry/index.mdx#core-metrics)
...
the `max_request_size` option in the [listener stanza](../configuration/listener/tcp.mdx). It defaults to 32 MiB. This value, minus the overhead of
...
[`max_request_duration`](../configuration/listener/tcp.mdx), which defaults to
...
The environment variable [`VAULT_CLIENT_TIMEOUT`](../commands/index.mdx#vault_client_timeout) sets a client-side maximum duration as well,
...
[listener stanza](../configuration/listener/tcp.mdx). It defaults to
...
[listener stanza](../configuration/listener/tcp.mdx) is used to limit the
...
A systemwide [maximum TTL](../configuration/index.mdx#max_lease_ttl), and a
...
[`vault.expire.num_leases`](telemetry/index.mdx#token-identity-and-lease-metrics) metric.

### External plugin limits

The [plugin system](../plugins/index.mdx) launches a separate process
