...
description: |-
  Learn about the telemetry data available in OpenBao
---

# Telemetry

The OpenBao server process collects various runtime metrics about the performance
...
Telemetry from OpenBao must be streamed and stored in metrics aggregation
...
You can view raw telemetry data for debugging purposes by interrupting the OpenBao
process with `USR1` (on *nix) or `BREAK` (on Windows). When the OpenBao process
