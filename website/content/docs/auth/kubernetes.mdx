...
1. Use the `/config` endpoint to configure OpenBao to talk to Kubernetes. Use
   ...
   **Note:** The pattern OpenBao uses to authenticate Pods depends on sharing
   ...
   OpenBao](../internals/security.mdx), this is allowable because OpenBao is part
...
[here](jwt/oidc-providers/kubernetes.mdx#specifying-ttl-and-audience) for an example.
...
[k8s-jwt-auth]: jwt/oidc-providers/kubernetes.mdx
