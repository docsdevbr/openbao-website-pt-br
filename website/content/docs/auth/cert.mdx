...
description: >-
  The "cert" auth method allows users to authenticate with OpenBao using TLS
  client certificates.
---
...
Please note that to use this auth method, `tls_disable` and `tls_disable_client_certs` must be false in the OpenBao
...
including OpenBao's `pki` method, multiple CRLs can successfully be used as
...
**NOTE** The `-ca-cert` value used here is for the OpenBao TLS Listener CA
...
**NOTE** The `--cacert` value used here is for the OpenBao TLS Listener CA
certificate, not the CA that issued the client authentication certificate. This
can be omitted if the CA used to issue the OpenBao server certificate is trusted
by the local system executing this command.
