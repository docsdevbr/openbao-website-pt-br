...
1. Configure Authorized [Redirect URIs](../index.mdx#redirect-uris).
...
The Google-specific handling that's used to fetch Google Workspace groups and user information in OpenBao uses
...
- `user_custom_schemas` `(string: <optional>)` - Comma-separated list of Google Workspace [custom schemas](https://developers.google.com/admin-sdk/directory/v1/guides/manage-schemas).
  Values set for Google Workspace users using custom schema fields will be fetched and made available as claims that can be used with [claim_mappings](/api-docs/auth/jwt#claim_mappings). Required if `fetch_user_info` is set to true.
