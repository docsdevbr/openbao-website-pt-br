...
Kubernetes can function as an OIDC provider such that OpenBao can validate its
...
[Kubernetes auth](../../kubernetes.mdx) which _does_ use the `TokenReview` API.
...
This method can be useful if Kubernetes' API is not reachable from OpenBao or if
...
When using the resulting token, you will need to set `bound_audiences=openbao`
