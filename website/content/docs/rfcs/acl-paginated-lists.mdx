...
Extend our [paginated list RFC](paginated-lists.mdx) by adding the ability to limit pagination via an ACL policy parameter, `pagination_limit` supplemented by `required_parameters = ["limit"]`.
