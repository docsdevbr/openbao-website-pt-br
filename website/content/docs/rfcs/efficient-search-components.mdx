...
sidebar_label: Components for efficient search over encrypted data
description: |-
  Constructs to build indexes on metadata for efficient search operations.
---
...
An alternative is leveraging third party tools to create an external index(es) via the audit logs. This approach does, however, leak sensitive information ([not directly](../audit/index.mdx#sensitive-information), unless log_raw=true is enabled).
