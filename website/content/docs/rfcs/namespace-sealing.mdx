...
However, other [seal configurations](../configuration/seal/index.mdx)
...
Sealing stanzas for KMS and PKCS#11 follow [the existing ones](../configuration/seal/index.mdx).
...
* In OpenBao, [Shamir's technique can be disabled](../internals/architecture.mdx), so the root key can be used
