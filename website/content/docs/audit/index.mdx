...
Devices](index.mdx#blocked-audit-devices) section below).  Therefore in order
...
While most strings are hashed, OpenBao does make some exceptions, such as auth and secrets, and users can enable additional exceptions using the [secrets enable](../commands/secrets/enable.mdx) command, and then tune it afterward.

**See also**:

* [secrets tune](../commands/secrets/tune.mdx)
* [auth enable](../commands/auth/enable.mdx)
* [auth tune](../commands/auth/tune.mdx)
...
[server configuration](../configuration/index.mdx) starting with OpenBao v2.3.2.
...
- `elide_list_responses` `(bool: false)` - See [Eliding list response
  bodies](index.mdx#eliding-list-response-bodies) below.

- `format` `(string: "json")` - Allows selecting the output format. The only
  valid value is `"json"`, which will be defaulted to if provided an empty
  string.
