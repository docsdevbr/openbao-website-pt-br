...
Consider using [declarative audit configuration](../configuration/audit.mdx)
instead.
...
the [main Audit Devices page](index.mdx#common-configuration-options), and
...
- `socket_type` `(string: "tcp")` - The socket type to use, any type compatible
  ...
  OpenBao may become unresponsive per [Blocked Audit Devices](index.mdx#blocked-audit-devices).
