...
sidebar_label: Process Supervisor Mode
description: >-
  OpenBao Agent's Process Supervisor Mode allows OpenBao secrets to be injected
  into a process via environment variables using Consul Template markup.
---

# OpenBao agent's process supervisor mode

OpenBao Agent's Process Supervisor Mode allows OpenBao secrets to be injected into
...
<!--   evaluating the [OpenBao Agent Sidecar Injector](../../platform/k8s/injector/index.mdx). -->

<!-- ::: -->

:::danger

OpenBao Agent's Process Supervisor Mode is in public beta. Please provide your
   feedback by opening a GitHub issue [here](https://github.com/openbao/openbao/issues).

:::

## Functionality

OpenBao Agent will inject secrets referenced in the `env_template` configuration
blocks as environment variables into the child process specified in the `exec` block.

When you start OpenBao Agent in process supervisor mode, it will wait until each
...
[`static_secret_render_interval`](template.mdx#static_secret_render_interval))
or dynamic secret being close to its expiration.

In many ways, OpenBao Agent will mirror the child process. Standard intput and
...
Agent's [generate-config](generate-config/index.mdx)
...
[templating language](template.mdx#templating-language)
as file templates but permits only a subset of
[its configuration parameters](template.mdx#template_configurations):
...
- `error_on_missing_key` `(bool: false)` - Exit with an error when accessing
  ...
  to "true". Also see
  [`exit_on_retry_failure` in global OpenBao Agent Template Config](template.mdx#interaction-between-exit_on_retry_failure-and-error_on_missing_key).
...
[`bao agent generate-config`](generate-config/index.mdx),
