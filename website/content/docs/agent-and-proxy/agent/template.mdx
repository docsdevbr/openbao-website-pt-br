...
description: >-
  OpenBao Agent's Template functionality allows OpenBao secrets to be rendered to
  files using Consul Template markup.
---

# OpenBao agent templates

OpenBao Agent's Template functionality allows OpenBao secrets to be rendered to files
or environment variables (via the [Process Supervisor Mode](process-supervisor.mdx))
...
The `template` stanza configures the OpenBao agent for rendering secrets to files
...
Engine](../../secrets/pki/index.mdx). Refer to the [Certificates](#certificates) section
...
The following is an example of a template that retrieves a generic secret from OpenBao's
KV store:
```
...
- `exit_on_retry_failure` `(bool: false)` - This option configures OpenBao Agent
...
- `static_secret_render_interval` `(string or integer: 5m)` - If specified, configures
  ...
  secrets. Uses [duration format strings](../../concepts/duration-format.mdx).
...
In another example `template_config` with [`error_on_missing_key` parameter in the template stanza](template.mdx#error_on_missing_key)
...
The parameter
[`error_on_missing_key`](template.mdx#error_on_missing_key) can be
...
- `create_dest_dirs`Δ `(bool: true)` - This option tells OpenBao Agent to create
...
- `error_on_missing_key` `(bool: false)` - Exit with an error when accessing
  ...
  to "true". Also see [`exit_on_retry_failure` in global OpenBao Agent Template Config](template.mdx#interaction-between-exit_on_retry_failure-and-error_on_missing_key).
...
The OpenBao Agent templating automatically renews and fetches secrets/tokens.
Unlike [OpenBao Agent caching](caching/index.mdx), the behavior of how OpenBao Agent
templating does this depends on the type of secret or token. The following is a
high level overview of different behaviors.

### Renewable secrets

If a secret or token is renewable, OpenBao Agent will renew the secret after 2/3
...
This can be configured using Template config [static_secret_render_interval](template.mdx#static_secret_render_interval).
Non-renewable secrets include (but not limited to) [KV Version 2](../../secrets/kv/kv-v2.mdx).

### Non-Renewable leased secrets

If a secret or token is non-renewable but leased, OpenBao Agent will fetch the secret when 85% of the
secrets time-to-live (TTL) is reached. Leased, non-renewable secrets include (but not limited
to) dynamic secrets such as [database credentials](../../secrets/databases/index.mdx) and [KV Version 1](../../secrets/kv/kv-v1.mdx).

### Static roles

If a secret has a `rotation_period`, such as a [database static role](../../secrets/databases/index.mdx#static-roles),
...
If a [certificate](../../secrets/pki/index.mdx) is rendered using the `pkiCert` template
...
If a [certificate](../../secrets/pki/index.mdx) is rendered using the `secret` template
function, OpenBao Agent template will have the following fetching and re-rendering
behaviors on certificates:
...
with [Process Supervisor Mode][process-supervisor]
...
[process-supervisor]: process-supervisor.mdx
