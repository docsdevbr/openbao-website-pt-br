...
description: AppRole Method for OpenBao Auto-Auth
---
...
- `secret_id_response_wrapping_path` `(string: optional)` - If set, the value
  ...
  Token](../../../concepts/response-wrapping.mdx)
...
An example configuration, using approle to enable [auto-auth](../index.mdx)
and creating both a plaintext token sink and a [response-wrapped token sink file](../index.mdx#wrap_ttl), follows:
