...
The [`listener` stanza](../agent/index.mdx#listener-stanza) for OpenBao Proxy configures a listener for OpenBao Proxy. If
its `role` is not set to `metrics_only`, it will act as a proxy for the OpenBao server that
has been configured in the [`vault` stanza](index.mdx#vault-stanza) of Proxy Agent. This enables access to the OpenBao
