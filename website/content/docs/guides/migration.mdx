...
sidebar_label: In-Place Migration Guide
description: >-
  This page contains a guide to migrate from HashiCorp Vault Community Edition to OpenBao
---

# In-Place Migration from Vault CE

This page contains a guide to migrate a HashiCorp Vault Community Edition
cluster to OpenBao.
...
Check [known-issues](known-issues.mdx) documentation
...
- OpenBao uses a different type of
  [tokens](concepts/tokens.mdx#token-formats)
...
Make a backup of your data! A [raft snapshot](commands/operator/raft.mdx#snapshot)
...
Make sure to have OpenBao [installed](install.mdx) and
[configured](configuration/index.mdx), _but not started_
...
not use `mlock` [since 2.0.0](rfcs/mlock-removal.mdx).
...
In this guide we do not rely on [Autopilot](commands/operator/raft.mdx#autopilot)
