...
revision: 06390c8714d41ebef7af1e17b685853c3bc84853
status: ready
...
1. Instalar a partir de um [gerenciador de pacotes](#gerenciador-de-pacotes).

2. Implantar usando um [registro de contêineres](#registros-de-contêineres).

3. Usar um [binário pré-compilado](#binários-pré-compilados).

4. Instalar [a partir do código-fonte](#compilando-a-partir-do-código-fonte).
...
Os pacotes estão disponíveis para download direto e por meio de gerenciadores de
pacotes nativos na página de [downloads](/downloads).

Além disso, os pacotes do OpenBao estão disponíveis diretamente no RHEL e no
Fedora.
Para o RHEL (e derivados), eles estão no EPEL, que pode ser habilitado por:

```shell-session
$ dnf install -y epel-release
```

e então instalado no RHEL ou Fedora por:
